...
def getControl(search_term, weeksback):
    ...
    while(back < oldestdate):
        ...
        index = 0
        for j in range(100):
            if (getTime(res[j][3]) <= oldestdate):
                index = j
        oldestdate = getTime(res[index][3])
        #oldestdate = getTime(res[99][3])
        print('oldestdate: ' + str(oldestdate))
        print('back: ' + str(back))
    #index = 0
    #for i in range(100):
...
def query(search_term, offset, article_count=100, from_date = 1262304000, subscription_key="db529dd884ae4732a2bf1a453aa66bb1"): #use epoch time
